...
/**
 * WEBHOOK FLYINGDONKEYS - DESENVOLVIMENTO V2
 * dev.bpsegurosimediato.com.br/webhooks/add_flyingdonkeys_dev.php
 * 
 * Versão de desenvolvimento com API V2, logging avançado e validação de signature
 * Baseado no add_travelangels_dev.php mas apontando para desenvolvimento FlyingDonkeys
 * 
...
// ✅ SECRET DO WEBFLOW DE DESENVOLVIMENTO (obtido do Webflow Dashboard)
$WEBFLOW_SECRET_TRAVELANGELS = 'ce051cb1d819faac5837f4e47a7fdd8cf2a8b248a2b3ecdb9ab358cfb9ed7990';
$DEBUG_LOG_FILE = '/var/www/html/logs/flyingdonkeys_dev.txt';
...
// Função para validar signature do Webflow (API V2) - DESENVOLVIMENTO
function validateWebflowSignatureProd($payload, $signature, $timestamp, $secret)
{
    // Em desenvolvimento, signature pode ser opcional
    if (empty($signature) || empty($timestamp)) {
        logDevWebhook('signature_validation', ['status' => 'missing', 'reason' => 'signature_or_timestamp_empty'], false);
        return false;
    }
...
try {
    ...
    if ($leadIdFromPayload) {
        ...
        if (empty($leadIdFromPayload)) {
            logDevWebhook('update_error_validation', [
        ...
        try {
            ...
            logDevWebhook('patch_request_starting', [
                'lead_id' => $leadIdFromPayload,
                'patch_url' => 'Lead/' . $leadIdFromPayload,
            ...
            logDevWebhook('patch_request_success', [
                'lead_id' => $leadIdFromPayload,
            ...
            logDevWebhook('patch_request_error', [
                'lead_id' => $leadIdFromPayload,
    ...
    try {
        logDevWebhook('flyingdonkeys_lead_creation_started', [
            'email' => $email,
            'name' => $name,
        ...
        logDevWebhook('flyingdonkeys_api_response', [
        ...
        if (!$leadIdFlyingDonkeys) {
            logDevWebhook('flyingdonkeys_lead_creation_missing_id', [
    ...
    if ($opportunityIdFromPayload) {
        ...
        try {
            ...
            logDevWebhook('patch_opportunity_request_starting', [
                'opportunity_id' => $opportunityIdFromPayload,
            ...
            if (empty($opportunityIdFromPayload)) {
                logDevWebhook('update_opportunity_error_validation', [
            ...
            // ✅ LOG APÓS SUCESSO DO PATCH
            logDevWebhook('patch_opportunity_request_success', [
                'opportunity_id' => $opportunityIdFromPayload,
            ...
            logDevWebhook('patch_opportunity_request_error', [
                'opportunity_id' => $opportunityIdFromPayload,
                'error_message' => $errorMessage,
