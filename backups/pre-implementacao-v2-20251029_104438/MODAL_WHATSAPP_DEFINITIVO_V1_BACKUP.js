...
$(function() {
    ...
    if (celularDigits === 9 && typeof validarTelefoneAsync === 'function') {
      ...
      validarTelefoneAsync($(MODAL_CONFIG.fieldIds.ddd), $(this)).then(res => {
        ...
        if (!res.ok) {
          ...
          // ✅ NOVO: Registrar primeiro contato no EspoCRM após validação bem-sucedida
          if (!initialRegistrationAttempted) {
            ...
            console.log('📞 [MODAL] Registrando primeiro contato no EspoCRM...');
            
            registrarPrimeiroContatoEspoCRM(ddd, celular, gclid)
              .then(result => {
                if (result.success) {
                  console.log('✅ [MODAL] Primeiro contato registrado:', result.id || 'sem ID');
                } else {
                  console.warn('⚠️ [MODAL] Erro ao registrar primeiro contato (não bloqueante):', result.error);
                }
              })
              .catch(error => {
                console.warn('⚠️ [MODAL] Exceção ao registrar primeiro contato (não bloqueante):', error);
              });
          }
        }
      }).catch(_ => hideLoading());
      ...
      // ✅ NOVO: Registrar primeiro contato mesmo sem validação via API (se formato está OK)
      if (celularDigits === 9 && dddDigits === 2 && !initialRegistrationAttempted) {
        ...
        console.log('📞 [MODAL] Registrando primeiro contato no EspoCRM (sem API)...');
  ...
  $form.on('submit', async function(e) {
    ...
    logEvent('whatsapp_modal_submit_start', {
      has_cpf: !!dados.CPF,
      has_nome: !!dados.NOME,
      has_placa: !!dados.PLACA
    }, 'info');
    ...
    try {
      ...
      // 1. Atualizar lead no EspoCRM (em paralelo com Octadesk)
      // 2. Enviar mensagem via Octadesk (em paralelo com EspoCRM)
      const [espocrmResult, octadeskResult] = await Promise.all([
        atualizarLeadEspoCRM(dados, espocrmId),
        enviarMensagemOctadesk(dados)
      ]);
      
      // 3. Registrar conversão no Google Ads (não é async, mas pode executar em paralelo)
      ...
      if (espocrmResult.success) {
        ...
        console.warn('⚠️ [MODAL] Erro ao atualizar lead (não bloqueante):', espocrmResult.error);
        logEvent('whatsapp_modal_espocrm_update_final_failed', { error: espocrmResult.error }, 'warning');
      }
      
      if (octadeskResult.success) {
        console.log('✅ [MODAL] Mensagem enviada via Octadesk');
        logEvent('whatsapp_modal_octadesk_final_success', {}, 'info');
      } else {
        console.warn('⚠️ [MODAL] Erro ao enviar mensagem (não bloqueante):', octadeskResult.error);
        logEvent('whatsapp_modal_octadesk_final_failed', { error: octadeskResult.error }, 'warning');
      }
      
      // Esconder loading
      hideLoading();
      
      // Sempre permitir abrir WhatsApp, mesmo se houver erros
      console.log('✅ [MODAL] Processo concluído, abrindo WhatsApp');
      
      logEvent('whatsapp_modal_submit_success', {
        espocrm_success: espocrmResult.success,
        octadesk_success: octadeskResult.success
      }, 'info');
    ...
    $modal.fadeOut(300, function() {
      ...
      logEvent('whatsapp_modal_submit_exception', { error: error.message }, 'error');
