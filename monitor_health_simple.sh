...
check_health() {
    ...
    if [ "$http_code" = "200" ]; then
        # Extrair status usando jq se disponível, senão usar método simples
        if command -v jq >/dev/null 2>&1; then
            status=$(jq -r '.status' /tmp/health_response.json 2>/dev/null)
        else
            # Método simples: procurar pela primeira ocorrência de "status"
            status=$(grep -o '"status":"[^"]*"' /tmp/health_response.json | head -1 | cut -d'"' -f4)
        fi
