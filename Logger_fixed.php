...
class Logger {
    ...
    private function buildLogEntry($input) {
        ...
        // Converter timestamp ISO para formato MySQL compatível
        $clientTimestamp = $input['timestamp'] ?? $timestamp->format('c');
        if (isset($input['timestamp'])) {
            try {
                $clientDateTime = new DateTime($input['timestamp']);
                $clientTimestamp = $clientDateTime->format('Y-m-d H:i:s');
            } catch (Exception $e) {
                // Se falhar, usar timestamp atual
                $clientTimestamp = $timestamp->format('Y-m-d H:i:s');
            }
        } else {
            $clientTimestamp = $timestamp->format('Y-m-d H:i:s');
        }
