#!/bin/bash
# Teste RPA Principal com monitoramento em tempo real
...
# Iniciar sessão
log_info "Iniciando sessão RPA Principal..."
...
while true; do
    ...
    if [ "$STATUS" = "success" ] && [ "$ETAPA" = "15" ]; then
        ...
        if [ "$RESULTADOS" = "true" ]; then
            log_success "Cálculo final capturado!"
            ...
            RESULT_FILES=$(find /opt/imediatoseguros-rpa/ -name "dados_planos_seguro_*.json" -newer "$PROGRESS_FILE" 2>/dev/null | head -1)
            if [ ! -z "$RESULT_FILES" ]; then
                log_success "Arquivo de resultados gerado: $RESULT_FILES"
            else
                log_warning "Arquivo de resultados não encontrado"
            fi
            break
        fi
    fi
...
log_success "Teste RPA Principal concluído com sucesso"
