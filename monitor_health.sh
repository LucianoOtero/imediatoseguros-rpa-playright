...
check_health() {
    ...
    if [ "$http_code" = "200" ]; then
        # Verificar se resposta contém status ok
        if grep -q '"status":"ok"' /tmp/health_response.json 2>/dev/null; then
            log_message "Health check $environment: OK"
            return 0
        elif grep -q '"status":"warning"' /tmp/health_response.json 2>/dev/null; then
            log_message "Health check $environment: WARNING"
            return 1
        else
            log_message "Health check $environment: ERROR (status não ok)"
            return 2
        fi
    else
