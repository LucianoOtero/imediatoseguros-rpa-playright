        ...
        for i, elem in enumerate(todos_elementos):
            try:
                ...
                if (texto.count('R$') >= 2 or 
                    ...
                    exibir_mensagem(f"📋 CONTAINER {i+1}: {len(texto)} caracteres - {texto[:100]}...")
            except:
                continue
        ...
        for i, elemento in enumerate(tabelas_planos):  # Processar todos os containers
            try:
                ...
                if "plano recomendado" in tabela_text.lower():
                    ...
                    exibir_mensagem(f"⚠️ TIPO DE PLANO NÃO IDENTIFICADO - Texto: {tabela_text[:200]}...")
                    continue
                ...
                if len(linhas) >= indice_inicio + 8:  # Mínimo de 8 campos após título
                    try:
                        ...
                        # 7-9. Processar ícones de cobertura (Assistência, Vidros, Carro Reserva) - ABORDAGEM CORRIGIDA
                        exibir_mensagem("🔍 Detectando ícones de cobertura diretamente no container...")
                        try:
                            # Detectar ícones diretamente no container (baseado na versão anterior que funcionava)
                            ...
                            # Mapear ícones por posição direta (CORRIGIDO)
