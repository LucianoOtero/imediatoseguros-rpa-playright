<?php
/**
 * TESTE RPA REAL - VERSÃO SIMPLIFICADA
...
echo "    <title>Teste RPA Real</title>";
echo "    <style>";
...
echo "        .log { background: #f8f9fa; padding: 15px; margin-top: 20px; max-height: 300px; overflow-y: auto; }";
echo "        .log-entry { margin: 5px 0; padding: 5px; background: white; border-radius: 3px; }";
echo "    </style>";
echo "</head>";
echo "<body>";
echo "    <div class='container'>";
echo "        <h1>🎭 Teste RPA Real</h1>";
echo "        <p>Execute o RPA Python real e acompanhe o progresso</p>";
echo "";
echo "        <div class='progress-bar'>";
echo "            <div class='progress-fill' id='progress-bar'></div>";
echo "        </div>";
echo "        <div class='status' id='status'>Aguardando início...</div>";
echo "";
echo "        <div style='text-align: center;'>";
echo "            <button onclick='iniciarRPA()' id='btn-iniciar'>🚀 Iniciar RPA</button>";
...
echo "        let sessionId = 'rpa_' + Math.random().toString(36).substr(2, 9);";
echo "";
...
echo "            if (eventSource) {";
echo "                log('⚠️ RPA já está em execução');";
echo "                return;";
echo "            }";
echo "";
echo "            log('🚀 Iniciando RPA real...');";
echo "            log('🌐 Chrome será aberto automaticamente');";
echo "";
...
echo "";
echo "            const url = 'progress_sse_simples.php?session_id=' + sessionId;";
echo "            eventSource = new EventSource(url);";
echo "";
echo "            eventSource.onopen = function() {";
echo "                log('✅ Conexão estabelecida');";
echo "            };";
echo "";
...
echo "                } catch (error) {";
echo "                    log('❌ Erro ao processar: ' + error.message);";
echo "                }";
echo "            };";
echo "";
echo "            eventSource.onerror = function() {";
echo "                log('❌ Erro na conexão');";
echo "                pararTeste();";
echo "            };";
echo "        }";
echo "";
echo "        function processarMensagem(data) {";
echo "            log('📨 ' + (data.status || 'progresso') + ': ' + (data.message || data.status || 'atualizando'));";
echo "";
echo "            if (data.status === 'starting') {";
echo "                document.getElementById('status').textContent = '🚀 Iniciando RPA...';";
echo "                atualizarProgresso(0);";
echo "            } else if (data.status === 'started') {";
echo "                document.getElementById('status').textContent = '🌐 Chrome aberto - RPA executando...';";
echo "                log('🎭 Playwright abriu Chrome!');";
echo "            } else if (data.status === 'completed') {";
echo "                document.getElementById('status').textContent = '✅ RPA concluído!';";
echo "                atualizarProgresso(100);";
echo "                log('🎉 RPA executado com sucesso!');";
echo "                pararTeste();";
echo "            } else if (data.status === 'timeout') {";
echo "                document.getElementById('status').textContent = '⏰ Timeout';";
echo "                log('⏰ Timeout atingido');";
echo "                pararTeste();";
echo "            } else if (data.status === 'error') {";
echo "                document.getElementById('status').textContent = '❌ Erro';";
echo "                log('❌ Erro: ' + data.message);";
echo "                pararTeste();";
...
echo "                log('📊 Etapa ' + etapa + ': ' + status + ' (' + percentual + '%)');";
echo "            }";
echo "        }";
echo "";
...
echo "            log('📱 Página carregada - Pronto para executar RPA');";
echo "            log('💡 Clique em \"Iniciar RPA\" para executar o Playwright');";
echo "        };";
echo "    </script>";
echo "</body>";
echo "</html>";
